...
//--------------------------------------------------------------------------------------------------------------------------------
		...
		#endif
			{
				...
				float scale = v.texcoord1.y + 1; // Hack in a place to fix ring scale. Should default to behaving normal. -- Tristan
				
				//--------------------------------
				// procedural ring

				float ringWidth = v.texcoord2.x * scale;
				float ringSmooth = v.texcoord2.y * scale;
				float ringRotation = v.texcoord2.z;
				float particleSize = v.texcoord2.w * scale;
